...
public class JavaCountLines extends AbstractCountLines {
    ...
    protected int count(String path) {
        ...
        //源文件转成的字符串
        StringBuilder content = getContent(path);
        if (content == null)
            return -1;
            ...
            if (content.charAt(currentCharacter) == '\n') {
                ...
                if (content.charAt(currentCharacter + 1) == '/' ||
                    ...
                    //处理"/**/"风格的注释
                        ...
                        while (currentCharacter < content.length()) {
                            if (currentCharacter + 1 < content.length() &&
                                    content.charAt(currentCharacter) == '/' &&
                                    content.charAt(currentCharacter + 1) == '*')
                                content.setCharAt(currentCharacter, ' ');
                            while (currentCharacter < content.length() &&
                                    content.charAt(currentCharacter) != '*') {
                                //遇到换行符就扫描该行是否有效
                                if (content.charAt(currentCharacter) == '\n' &&
     ...
     * @return 是否是一个有效的行
     */
    protected boolean isValidLine(StringBuilder content, int currentCharacter, boolean inComment) {
        if (inComment) {
            if (content.charAt(currentCharacter) == '\n')
                currentCharacter--;
     ...
     * @return 注释后的下一个字符位置
     */
    protected int processSingleLineComment(StringBuilder content, int currentCharacter) {
        if (currentCharacter + 2 < content.length() &&
                content.charAt(currentCharacter + 2) != '\n')
            currentCharacter = currentCharacter + 2;
        while (currentCharacter < content.length() &&
                content.charAt(currentCharacter) != '\n') {
            if (content.charAt(currentCharacter) == '/')
